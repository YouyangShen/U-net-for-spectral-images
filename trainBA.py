...
def train_net(net,
    ...
    experiment.config.update(dict(epochs=epochs, batch_size=batch_size, learning_rate=learning_rate,
                                  ...
                                  amp=amp,loss_function=loss_function))
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    ...
                    if loss_function == False:
                        loss =  dice_loss(F.softmax(masks_pred, dim=1).float(),
                                           ...
                                           multiclass=False)  ##criterion(masks_pred, true_masks) \
                    else:
...
def get_args():
    ...
    parser.add_argument('--loss', '-q', type=str, default=False, help='loss is cross entrophy')
...
if __name__ == '__main__':
    ...
    net = UNet(n_channels=7, n_classes=2, bilinear=True)
